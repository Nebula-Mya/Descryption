...
class Dam(card.BlankCard) : # only given by a sigil
    def __init__(self, blank_cost=True, sigils=None) :
        if sigils is None :
            sigils = ['','']
...
class Pronghorn(card.BlankCard) :
    def __init__(self, blank_cost=False, sigils=None) :
        if random.randint(0,1) == 0 and sigils is None :
...
class Smoke(card.BlankCard) : # only given in campaign
    def __init__(self, blank_cost=False, sigils=None) :
        if sigils is None :
            sigils = ['worthy sacrifice', ''] # will have the bone king sigil once bones are implemented
...
class Bloodhound(card.BlankCard) : 
    def __init__(self, blank_cost=False, sigils=None) :
        if sigils is None :
            sigils = ['',''] # will have guardian sigil once implimented
...
class MoleMan(card.BlankCard) : # lane shift will be replaced with burrower sigil once implemented
    def __init__(self, blank_cost=False, sigils=None) :
        if sigils is None :
            sigils = ['lane shift left','mighty leap']
...
class Tree(card.BlankCard) : # terrain card
    def __init__(self, blank_cost=False, sigils=None, level=0) :
        if sigils is None :
            sigils = ['mighty leap','']
        match level :
